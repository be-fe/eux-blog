...
--------------------------------------------------------------------
s  exp(11位)     frac(52位，注意存储时候前面的1会舍去)
```
...
一堆理论扯完之后，我们来看 `0.1 + 0.2` 的运算过程。首先对阶，0.1 的阶码为 -4，0.2 的阶码为 -3，依照小阶对大阶的原则，我们需要将 0.1 的阶码变为 -3，因此其尾数部分需要右移一位，对阶之后 0.1 的存储为：
```
0  01111111100  1100110011001100110011001100110011001100110011001100
--------------------------------------------------------------------
s  exp(阶码-3)   frac(由于尾数隐含了一个最高位是1的条件，因此移位后最高位是1而不是0)
```
...
```javascript
const a = "01100110011001100110011001100110011001100110011001100".split('');
const b = "11001100110011001100110011001100110011001100110011001".split('');
const res = [];
let flag = 0;
...
得出：
```
   0  01111111100   (0)1100110011001100110011001100110011001100110011001100
+  0  01111111100   (1)1001100110011001100110011001100110011001100110011001
-------------------------------------------------------------------
=  0  01111111100  100110011001100110011001100110011001100110011001100101
```

这里计算时，进位2位，去除原来最高位默认的1，相当于阶码部分加1，即阶码由原来的 -3 变为 -2，所以阶码部分为 `01111111101`。而尾数部分，去除最高位1，最后一位1，进行舍入（最低位进位），得到52位新的二进制表示： `0011001100110011001100110011001100110011001100110011`
所以，最终的计算结果在计算机中的存储表达如下：
```
0  1111111101  0011001100110011001100110011001100110011001100110011
```
即 0.1+0.2 的二进制结果 `2^-2 * 1.0011001100110011001100110011001100110011001100110011`
将其转换成十进制数为：0.3000000000000000444089209850062616169452667236328125，由于精度问题，只取到 0.30000000000000004。
...
## 写在最后
好了，我们已经知道为什么 `0.1 + 0.2 !== 0.3` 的原因了，主要由于0.1和0.2 转为二进制的时候为无限循环小数，而计算机的存储位置有限因此会做一定的截取舍入处理，再进行加减就有一定的误差了。那要怎么在 js 中尽可能准确的计算出结果呢，敬请期待下回分解～
