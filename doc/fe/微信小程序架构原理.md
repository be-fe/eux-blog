...
![](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-a.jpg)    
原生长这样的  
### 编译过程  
...
![](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-b.jpg)   

3. wx 核心对象，包含了 wx 对象下的 api。但是这里的 api 数量远远少于官方的 api 文档数量。
![](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-c.jpg)
我们可以再代码里面发现，wx 下注册的 api 最终都会调用 WeixinJSBridge 方法，这个方法。应该是在打包的时候端上注入的。然而，我们也可以在 WAServeice.js 中找到该方法的定义。  
![](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-d.jpg)   
...
5. 接都是对 exparser 对象的处理，包括注册 WeixinJSBridge 全局事件，Virtual dom 算法实现，样式注入等。介绍几个组件重要的内容  
     * exparser.registerBehavior 注册组件基础行为，供组件继承。
     [](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-e.jpg)    

     * exparser.registerElement 为各种内置组件，注册模板，行为，属性，监听器等内容

![](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-f.jpg)
...
![](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-h.jpg)   

综上，微信小程序的界面有部分组件使用原生方式实现的，Native组件层在WebView层之上。大部分还是用前端实现的，这样解释了微信小程序的一个bug。
> 微信官方文档：  
> ![](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-i.jpg)   
...
![](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-j.jpg)   

WeixinJSBridge 真正发送处理数据请求的时这端代码；如果当前环境是 ios, 那么调用 WKWebview的 window.webkit.messageHandlers.invokeHandler.postMessage。如果所处环境是 android 则调用 WeixinJSCore.invokeHandler(调用的时候，默认会带上当前webviewID)。  
#### WAService.js  
...
到这里我们得出结论，小程序的架构方案：
![](https://eux-public.bj.bcebos.com/2018/08/15/tgy-wx-k.jpg)     
