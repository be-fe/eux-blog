...
`WeixinJSBridge` 真正发送处理数据请求的是这端代码；如果当前环境是 `ios`, 那么调用 `WKWebview` 的 `window.webkit.messageHandlers.invokeHandler.postMessage`。如果所处环境是 `android` 则调用 `WeixinJSCore.invokeHandler` (调用的时候，默认会带上当前 `webviewID`)。 

#### WAService.js  

在对 `WeixinJSBridge.js` 分析中，我们并没有发现前端的通讯功能呢，路由能力，数据绑定等机制。进一步查看找到了一个 `WAService.js` 文件。 
